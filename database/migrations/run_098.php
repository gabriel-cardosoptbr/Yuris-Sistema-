<?php
/**
 * Runner da migration 098 (seed do prompt universal v2 do agente).
 *
 * Uso local: C:\xampp\php\php.exe database/migrations/run_098.php
 * Uso prod:  docker exec -i yuris_app php /var/www/html/database/migrations/run_098.php
 *
...
echo "== Migration 098: prompt universal v2 ==\n";
echo "DB: " . $pdo->query('SELECT DATABASE()')->fetchColumn() . "\n----\n";

$seed = require __DIR__ . '/../seeds/ai_prompt_v2.php';

// Schema (secao 23) reaproveitado do prompt global ja semeado na 097.
$schema = $pdo->query(
...
if ($id) {
    ...
    echo "  [ok] prompt v2 atualizado (id={$id}) e ativado\n";
} else {
    $pdo->prepare(
    ...
    echo "  [ok] prompt v2 inserido e ativado (id=" . $pdo->lastInsertId() . ")\n";
}

$len = mb_strlen($seed['template']);
echo "  [info] tamanho do prompt v2: {$len} caracteres (~" . (int)round($len / 4) . " tokens estimados)\n";
