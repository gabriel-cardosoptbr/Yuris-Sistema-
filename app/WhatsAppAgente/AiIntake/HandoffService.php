...
final class HandoffService
{
    public static function run(\PDO $pdo, array $cfg, array $p): array
    {
        ...
        if (!$taskId && ($hcfg['create_task'] ?? false) && $boardId) {
            try {
                require_once __DIR__ . '/../../Tarefas/Task.php';
                require_once __DIR__ . '/../../Tarefas/TaskColumn.php';
                $col = \App\Tarefas\TaskColumn::initialColumn((int)$boardId);
                $colId = $col['id'] ?? null;
                if ($colId) {
        ...
        try {
            ...
            \App\Master\Account::audit($accountId, 'ai_intake.handoff', [
