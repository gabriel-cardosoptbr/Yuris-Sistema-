...
final class Anonymizer
{
    ...
    public static function searchAllModules(
        ...
        if (!class_exists('App\\Lgpd\\LgpdRequest')) {
            require_once __DIR__ . '/LgpdRequest.php';
        }
        ...
        \App\Lgpd\LgpdRequest::addEvent($requestId, 'busca_completa',
    ...
    private static function searchUsers(\PDO $pdo, int $reqId, string $email, ?string $cpf, ?string $tel, int $byUid): int
    {
        ...
        foreach ($rows as $r) {
            if ($r['nome'])     \App\Lgpd\LgpdRequestFinding::record($reqId, 'usuarios', 'users', (int)$r['id'], 'pii_basica', $r['nome'], 'nome', (int)$r['account_id'], 'contrato', 'Cadastro como usuario do sistema');
            if ($r['login'])    \App\Lgpd\LgpdRequestFinding::record($reqId, 'usuarios', 'users', (int)$r['id'], 'pii_basica', $r['login'], 'email', (int)$r['account_id'], 'contrato', 'E-mail de login');
            if ($r['telefone']) \App\Lgpd\LgpdRequestFinding::record($reqId, 'usuarios', 'users', (int)$r['id'], 'pii_basica', $r['telefone'], 'telefone', (int)$r['account_id'], 'contrato', 'Telefone de contato');
            if ($r['oab'])      \App\Lgpd\LgpdRequestFinding::record($reqId, 'usuarios', 'users', (int)$r['id'], 'documento', $r['oab'], 'oab', (int)$r['account_id'], 'contrato', 'Registro OAB');
        }
        \App\Lgpd\LgpdRequestModule::record($reqId, 'usuarios', $byUid, count($rows),
    ...
    private static function searchContatos(\PDO $pdo, int $reqId, string $email, ?string $cpf, ?string $tel, int $byUid): int
    {
        ...
        foreach ($rows as $r) {
            if ($r['nome'])     \App\Lgpd\LgpdRequestFinding::record($reqId, 'contatos', 'contatos', (int)$r['id'], 'pii_basica', $r['nome'], 'nome', (int)$r['account_id'], 'contrato', 'Contato cadastrado');
            if ($r['telefone']) \App\Lgpd\LgpdRequestFinding::record($reqId, 'contatos', 'contatos', (int)$r['id'], 'pii_basica', $r['telefone'], 'telefone', (int)$r['account_id'], 'contrato', 'Telefone do contato');
            if ($r['email'])    \App\Lgpd\LgpdRequestFinding::record($reqId, 'contatos', 'contatos', (int)$r['id'], 'pii_basica', $r['email'], 'email', (int)$r['account_id'], 'contrato', 'E-mail do contato');
        }
        \App\Lgpd\LgpdRequestModule::record($reqId, 'contatos', $byUid, count($rows),
    ...
    private static function searchCards(\PDO $pdo, int $reqId, string $email, ?string $tel, int $byUid): int
    {
        ...
        foreach ($rows as $r) {
            if ($r['cliente_nome'])      \App\Lgpd\LgpdRequestFinding::record($reqId, 'leads_cards', 'cards', (int)$r['id'], 'pii_basica', $r['cliente_nome'], 'cliente_nome', (int)$r['account_id'], 'interesse_legitimo', 'Lead/prospect do funil');
            if ($r['telefone_whatsapp']) \App\Lgpd\LgpdRequestFinding::record($reqId, 'leads_cards', 'cards', (int)$r['id'], 'pii_basica', $r['telefone_whatsapp'], 'telefone_whatsapp', (int)$r['account_id'], 'interesse_legitimo', 'Contato WhatsApp do lead');
            if ($r['email'])             \App\Lgpd\LgpdRequestFinding::record($reqId, 'leads_cards', 'cards', (int)$r['id'], 'pii_basica', $r['email'], 'email', (int)$r['account_id'], 'interesse_legitimo', 'E-mail do lead');
        }
        \App\Lgpd\LgpdRequestModule::record($reqId, 'leads_cards', $byUid, count($rows),
    ...
    private static function searchProcessos(\PDO $pdo, int $reqId, string $email, ?string $cpf, int $byUid): int
    {
        ...
        foreach ($rows as $r) {
            if ($r['cpf_cnpj_parte_contraria']) {
                \App\Lgpd\LgpdRequestFinding::record($reqId, 'processos', 'processos', (int)$r['id'], 'juridico_processual',
            ...
            if ($r['numero_cnj']) {
                ...
                \App\Lgpd\LgpdRequestFinding::record($reqId, 'processos', 'processos', (int)$r['id'], 'metadado',
                    $r['numero_cnj'], 'numero_cnj',
                    (int)$r['account_id'], 'exercicio_direito', 'Numero CNJ do processo vinculado');
            }
        }
        \App\Lgpd\LgpdRequestModule::record($reqId, 'processos', $byUid, count($rows),
    ...
    private static function searchWhatsApp(\PDO $pdo, int $reqId, string $email, ?string $tel, int $byUid): int
    {
        ...
        foreach ($rows as $r) {
            if ($r['contact_name']) {
                \App\Lgpd\LgpdRequestFinding::record($reqId, 'whatsapp', 'whatsapp_messages', (int)$r['id'], 'comunicacao',
                    $r['contact_name'], 'contact_name', (int)$r['account_id'],
                    'interesse_legitimo', 'Mensagem WhatsApp (' . $r['message_type'] . ')');
            }
        }
        \App\Lgpd\LgpdRequestModule::record($reqId, 'whatsapp', $byUid, count($rows),
    ...
    private static function searchChatInterno(\PDO $pdo, int $reqId, string $email, int $byUid): int
    {
        ...
        foreach ($rows as $r) {
            \App\Lgpd\LgpdRequestFinding::record($reqId, 'chat_interno', 'chat_mensagens', (int)$r['id'], 'comunicacao',
                'msg-' . $r['id'], 'mensagem', (int)$r['account_id'],
                'interesse_legitimo', 'Mensagem enviada no chat interno em ' . $r['created_at']);
        }
        \App\Lgpd\LgpdRequestModule::record($reqId, 'chat_interno', $byUid, count($rows),
    ...
    private static function searchTaskAttachments(\PDO $pdo, int $reqId, string $email, int $byUid): int
    {
        ...
        foreach ($rows as $r) {
            \App\Lgpd\LgpdRequestFinding::record($reqId, 'task_attachments', 'task_attachments', (int)$r['id'], 'metadado',
                $r['file_name'] ?? 'arquivo', 'file_name', (int)($r['account_id'] ?? 0),
                'interesse_legitimo', 'Anexo enviado em ' . $r['created_at']);
        }
        \App\Lgpd\LgpdRequestModule::record($reqId, 'task_attachments', $byUid, count($rows),
    ...
    private static function searchFinanceiro(\PDO $pdo, int $reqId, string $email, int $byUid): int
    {
        ...
        foreach ($rows as $r) {
            \App\Lgpd\LgpdRequestFinding::record($reqId, 'financeiro', 'invoices', (int)$r['id'], 'financeiro',
                'invoice-' . $r['numero'], 'numero', (int)$r['account_id'],
                'cumprimento_contrato', 'Fatura ' . $r['status'] . ' R$ ' . number_format($r['amount_cents']/100, 2, ',', '.'));
        }
        \App\Lgpd\LgpdRequestModule::record($reqId, 'financeiro', $byUid, count($rows),
    ...
    private static function searchLogsAuditoria(\PDO $pdo, int $reqId, string $email, int $byUid): int
    {
        ...
        foreach ($rows as $r) {
            \App\Lgpd\LgpdRequestFinding::record($reqId, 'logs_auditoria', 'master_audit_log', (int)$r['id'], 'metadado',
                $r['acao'], 'acao', null,
        ...
        foreach ($rows as $r) {
            \App\Lgpd\LgpdRequestFinding::record($reqId, 'logs_auditoria', 'account_audit_log', (int)$r['id'], 'metadado',
        ...
        \App\Lgpd\LgpdRequestModule::record($reqId, 'logs_auditoria', $byUid, $totalLogs,
    ...
    private static function searchConsentimentos(\PDO $pdo, int $reqId, string $email, int $byUid): int
    {
        ...
        foreach ($rows as $r) {
            \App\Lgpd\LgpdRequestFinding::record($reqId, 'consentimentos', 'lgpd_consents', (int)$r['id'], 'metadado',
                $r['finalidade'], 'finalidade', (int)($r['account_id'] ?? 0),
                strtolower($r['base_legal']), 'Consentimento ' . $r['status'] . ' em ' . $r['concedido_em']);
        }
        \App\Lgpd\LgpdRequestModule::record($reqId, 'consentimentos', $byUid, count($rows),
    ...
    private static function searchAceitesTermos(\PDO $pdo, int $reqId, string $email, int $byUid): int
    {
        ...
        foreach ($rows as $r) {
            \App\Lgpd\LgpdRequestFinding::record($reqId, 'aceites_termos', 'term_acceptances', (int)$r['id'], 'metadado',
                ($r['tipo'] ?? '?') . ' v' . ($r['versao'] ?? '?'), 'tipo_versao', (int)($r['account_id'] ?? 0),
                'consentimento', 'Aceite via ' . ($r['contexto'] ?? '-') . ' em ' . $r['accepted_at']);
        }
        \App\Lgpd\LgpdRequestModule::record($reqId, 'aceites_termos', $byUid, count($rows),
